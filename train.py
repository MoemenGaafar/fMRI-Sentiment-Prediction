...
def load_and_match_roi_data(roi_folder, sentiment_df, num_TRs, start_from):
    ...
    X = np.empty(shape=(0,cube_size*cube_size*cube_size))
...
def train_and_evaluate_ROI(roi, processed_folder, train_lang, test_lang, num_TRs, start_from, width):
    train_data_folders = [os.path.join(processed_folder, folder_name) for folder_name in os.listdir(processed_folder) if train_lang in folder_name][:10]
    test_data_folders = [os.path.join(processed_folder, folder_name) for folder_name in os.listdir(processed_folder) if test_lang in folder_name]
    print("Training on ROI:", roi["name"])

    accuracies = []
    for i in range(len(train_data_folders)):
        train_subject_folders = copy.deepcopy(train_data_folders)
        if train_lang == test_lang:
            test_subject_folder = train_data_folders[i]
            train_subject_folders.remove(test_subject_folder)
        else:
            test_subject_folder = test_data_folders[i%len(test_data_folders)]

        print("Fold", i, "/", len(train_data_folders))
        print("Current test subject:", test_subject_folder)

        # Load and preprocess sentiment_df
        train_sentiment_df = load_labels(width, train_lang)
        test_sentiment_df = load_labels(width, test_lang)

        # Load roi data from all train subjects
        X_train = np.empty(shape=(0,cube_size*cube_size*cube_size))
        y_train = np.empty(shape=(0, 3))


        for subject_folder in train_subject_folders:
            roi_folder = os.path.join(subject_folder, "ROIs", roi["name"])
            X_subject, y_subject = load_and_match_roi_data(roi_folder, train_sentiment_df, num_TRs, start_from)
            X_train = np.vstack((X_train, X_subject))
            y_train = np.vstack((y_train, y_subject))

        # Load roi data from test subject
        test_roi_folder = os.path.join(test_subject_folder, "ROIs", roi["name"])
        X_test, y_test = load_and_match_roi_data(test_roi_folder, test_sentiment_df, num_TRs, start_from)
...
if __name__ == "__main__":
    ...
    for width in filter_width:
        if width == 0:
            results_file = os.path.join(results_folder, 'smoothing_10', 'LOSO', "results_"+train_lang+"_"+test_lang+"_avg_no_filter.json")
        else:
            results_file = os.path.join(results_folder, 'smoothing_10', 'LOSO', "results_"+train_lang+"_"+test_lang+"_avg_filter"+str(width)+".json")
        ...
        for roi in ROIs:
            ...
            for num_TRs in num_TRs_list:
                ...
                for start_from in start_from_list:
                    accuracies = train_and_evaluate_ROI(roi, processed_folder, train_lang, test_lang, num_TRs, start_from, width)
